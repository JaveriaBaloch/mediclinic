...
export async function rescheduleAppointmentsForEmptyDates(doctorId: string, removedRanges: { startDate: Date; endDate: Date }[]) {
    ...
    // Loop through removed vacation ranges to reschedule appointments
    for (const { startDate, endDate } of removedRanges) {
        // Check if any appointments fall within the removed vacation period
        for (const appointment of appointments) {
            const appointmentDate = appointment.appointmentTime;

            // Check if the appointment falls within the removed vacation period
            if (appointmentDate >= startDate && appointmentDate <= endDate) {
                // Reschedule logic: Move the appointment to the next available time
                ...
                break; // Exit the loop after rescheduling
            }
        }
    }
}
